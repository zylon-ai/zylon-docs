---
title: "Messages API"
description: "Aprende el formato de la request: roles, bloques de contenido, conversaciones multi-turn, system prompts, streaming y JSON estructurado."
---

La Messages API es el núcleo de ZylonGPT. Envías un historial de conversación y Zylon devuelve el siguiente mensaje del asistente como **lista de bloques de contenido**.
...
- `model`: nombre del modelo (comúnmente `"default"`)
...
Para generaciones largas (o si necesitas iniciar y observar después), usa Async Messages API:

- Start: `POST /messages/async`
- SSE: `GET /messages/async/{message_id}/stream`
- Status: `GET /messages/async/{message_id}/status`
...
<CardGroup cols={2}>
  ...
  <Card title="Async messages" icon="clock" href="/es/developer-manual/build-with-zylon/async-messages">
    Inicia mensajes async y luego streamea o consulta estado.
  </Card>
</CardGroup>
