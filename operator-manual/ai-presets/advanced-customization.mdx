...
├─ Model weights: 28GB compressed to ~14GB (quantized)
...
| GPT-OSS-20B (FP4)| 3 GB        | 3 / 24 = 0.125    | 0.12                |
| Embeddings        | 2.5 GB      | 2.5 / 24 = 0.104  | 0.10                |
| **Total**         | **20.5 GB** | **20.5 / 24**     | **0.85** (under 1.00 ✓) |
...
- Reducing memory allocation requires reducing `contextWindow` normally
</Warning>
  ...
  config:
    models:
      ...
      - id: llm
        name: qwen-3-14b-awq
        type: llm
        contextWindow: 4096              # Reduced from 8192 due to memory
        promptStyle: qwen
