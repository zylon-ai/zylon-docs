...
1. **Table mode**: Controls the table extraction strategy:
   - `accurate`: Maximum precision for complex tables. Higher processing time.
   - `fast`: Optimized for speed with good accuracy. Lower processing time.
   - `none`: Disables table extraction. No processing time.

2. **Do cell matching**: Enables matching of table cells to improve structure recognition. Recommended for complex tables.

3. **Force full page OCR**: Forces OCR processing on entire pages instead of selective regions. Use when standard extraction misses content.
