...
- clientId → Application client Id (from step 10)
- clientSecret → Secret value (from step 11)
- redirectUri → same value as step 4
- domain → Your sharepoint url
- tenantId → Directory tenant Id (from step 10)
...
- Check that `redirectUri` is the same as step 4 and that
