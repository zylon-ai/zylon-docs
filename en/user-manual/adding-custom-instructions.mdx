...
The maximum size of this field is controlled by your operator through `zylonWorkspace.projects.maxInstructionsLength` in `/etc/zylon/zylon-conf.yaml`. The default limit is `4000`, and the same limit is enforced in both the Workspace UI and the API. For operator-side configuration, see [Project custom instructions](/en/operator-manual/configuration/features/feat-project-custom-instructions).
