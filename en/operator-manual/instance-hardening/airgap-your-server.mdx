...
Examples of feature that *won't* be available without internet access are (list might grow in the future):
* AI Web search
* MCP connections
...
* Temporary disable the firewall, run the update, re-enable the firewall
...
| jq -r '.prefixes[]
         | select(.ipv4Prefix)
         | "sudo ufw allow from \(.ipv4Prefix) to any port 443"'
)
# And apply them
eval "$google_rules"
```
...
| flatten[]
| "sudo ufw allow from \(. ) to any port 443"' ServiceTags_Public_20260119.json
)
# And apply them
