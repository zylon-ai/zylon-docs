...
- Create a workspace member with the account ID in [Workspace members - Create a workspace member](/en/developer-manual/build-with-zylon/workspace-api/users/user).
...
| `scopes` | string[] | No | Allowed values: `Read.All`, `ReadWrite.All`, `Workspace.Read.All`, `Workspace.ReadWrite.All`, `PrivateGPT.Read.All`, and `PrivateGPT.ReadWrite.All`. If omitted, defaults to `Workspace.ReadWrite.All`; an explicit empty array returns `400`. |
| `gateway_id` | string | No | Required when `scopes` includes `PrivateGPT.Read.All` or `PrivateGPT.ReadWrite.All`. |
</Accordion>

Scopes on the same token are combined, then intersected with the owner account's role permissions. A token cannot elevate its owner, including a SuperAdmin token. Scope restrictions apply to API tokens only and do not change interactive sessions. See [Token Management](/en/developer-manual/get-started/token-management) for the access provided by each scope.

`Read.All` can be created without `gateway_id`, but its ZylonGPT read grants are usable only when the token carries that gateway context. In contrast, account-token creation returns `400` when either dedicated `PrivateGPT.*` scope is requested without `gateway_id`.
...
- **400**: `scopes` is explicitly empty, a PrivateGPT scope has no `gateway_id`, or a gateway token request contains a non-PrivateGPT scope.
