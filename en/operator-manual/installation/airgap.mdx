...
<Steps>
  <Step title="Stop Existing Zylon (If Applicable)">
    _Estimated time: 5 min_

    <Warning>
      Only perform this step if you're migrating on the same physical machine. Skip if deploying to a new airgap machine.
    </Warning>

    If migrating on the same machine, stop the current Zylon installation:
