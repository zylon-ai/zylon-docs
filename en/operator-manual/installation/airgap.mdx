...
<Steps>
  ...
  <Step title="Export Bundle">
    ...
    This creates `zylon-<version>-<so>-<arch>.tar` in your current directory. Additionally, it will generate
  ...
  <Step title="Transfer Files">
    ...
    **First installation:**
    - `zylon-<version>-<so>-<arch>.tar` (main bundle)
    - `zylon-initial.tar.gz` (CLI and config)
    - `zylon-ssl-certs.tar.gz` (SSL certs, if applicable)

    **Subsequent updates:**
    - `zylon-<version>-<so>-<arch>.tar` (main bundle only)

    **SCP Example**

    ```bash
    scp zylon-<version>-<so>-<arch>.tar user@offline-machine:/tmp/
...
<Steps>
  ...
  <Step title="On Online Machine: Create Update Bundle">
    ...
    **Include models (if applicable):**
    ...
    This generates `zylon-<new-version>-to-<old-version>-<so>-<arch>.tar` and displays the import/install commands to run on the offline machine as well.
  ...
  <Step title="Transfer Bundle to Offline Machine">
    ...
    **SCP Example:**
    ```bash
    scp zylon-<new-version>-to-<old-version>-<so>-<arch>.tar user@offline-machine:/tmp/
    ```
  ...
  <Step title="On Offline Machine: Import Update">
    ...
    ```bash
    cd /tmp
    sudo zylon-cli import zylon-<new-version>-to-<old-version>-<so>-<arch>.tar
    ```
