...
| `experimental.gemma3-24g`  | 24GB                | Gemma 3      | Alpha  |
...
| ------------------------- | ----------------------------------------- | -------- | ----------------------------- |
| `id`                      | Model identifier                          | Yes      | llm, embed                    |
| `name`                    | Custom model name                         | Yes      | string                        |
| `type`                    | Model type                                | Yes      | llm, embedding                |
| `contextWindow`           | Maximum context length                    | Yes      | integer                       |
| `modelRepo`               | HuggingFace model path                    | Yes      | string                        |
| `gpuMemoryUtilization`    | Fraction of GPU memory to use             | Yes      | 0.0-1.0                       |
| `samplingParams`          | Default sampling parameters               | No       | SamplingParams                |
| `reasoningSamplingParams` | Reasoning sampling parameters             | No       | SamplingParams                |
| `tokenizer`               | HuggingFace tokenizer path (llms only)                | Yes      | string                        |
| `promptStyle`             | Prompt formatting style (llms only)                   | Yes      | qwen, mistral, gemma, gpt-oss |
| `supportReasoning`        | Enable reasoning capabilities (llms only) | No       | boolean                       |
| `supportImage`            | Number of supported images (llms only)    | No       | integer                       |
| `supportAudio`            | Number of supported audios (llms only)    | No       | integer                       |
| `vectorDim`               | Vector dimensions (embeddings only)       | No       | integer                       |
