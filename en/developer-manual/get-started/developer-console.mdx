...
## Overview
Developer Console is the Backoffice area used to manage gateway-scoped ZylonGPT API tokens. These tokens call `/api/gpt/v1/*` through the selected gateway; they are not general Workspace or Backoffice API tokens.
...
Gateway token creation accepts `PrivateGPT.Read.All` and `PrivateGPT.ReadWrite.All`. An explicit empty legacy scope list maps to `PrivateGPT.ReadWrite.All`; any other requested scope returns `400`. See [Token Management](/en/developer-manual/get-started/token-management) for the scope model and the read-only allowlist.
