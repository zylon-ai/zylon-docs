...
<Steps>
  <Step title="Detener Zylon Existente (Si Aplica)">
    _Tiempo estimado: 5 min_

    <Warning>
      Solo realiza este paso si estás migrando en la misma máquina física. Omítelo si estás desplegando en una nueva máquina airgap.
    </Warning>

    Si migras en la misma máquina, detén la instalación actual de Zylon:

    ```bash
    sudo zylon-cli uninstall
    ```
  </Step>
