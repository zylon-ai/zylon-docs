...
| --- | --- | --- | --- |
| Path | `threadID` | Yes | Thread identifier. |
| Body | `thread_id` | Yes | Thread identifier. |
| Body | `pgpt` | Yes | Chat request body. |
| Body | `artifact_ids` | No | Artifact IDs to use as context. |
| Body | `all_artifacts` | No | Include all available artifacts when `true`. |
| Body | `agent` | No | Agent preset to use. |
</Accordion>
...
| Body | `name` | Yes | Thread name. |
| Body | `visibility` | Yes | `Public` or `Private`. |
</Accordion>
...
| --- | --- | --- | --- |
| Path | `projectID` | Yes | Project identifier. |
| Query | `state` | No | Comma-separated thread states: `Active` and `Deleted`. |
| Query | `include` | No | Comma-separated related data to include: `Account` and `ReadState`. |
...
```bash
curl "https://{BASE_URL}/api/v1/app/thread/{threadID}/interaction?page=1&page_size=20&include=Account" \
  -H "Authorization: Bearer {API_TOKEN}" \
  -H "x-org: {org_slug}"
```
