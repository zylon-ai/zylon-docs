---
title: "Messages"
description: "Requests de Messages con patrones sync, streaming y async."
---
...
| `content` | String o array de bloques. |

## Validar una solicitud

Usa `/messages/validate` para comprobar la forma del payload antes de enviar una solicitud real. Devuelve `valid: true` o una lista de errores de validación.
...
{/* <!-- ADDED FOR CLARITY --> */}
<Callout type="tip">
  Prefiere el campo top-level <code>system</code> para que las instrucciones se apliquen de forma consistente.
</Callout>
...
Los mensajes asíncronos usan **el mismo body** que los mensajes síncronos—solo cambia el endpoint. La diferencia principal está en la **respuesta**: sync devuelve el mensaje completo, async devuelve un `message_id` para stream o polling.

{/* <!-- RESTRUCTURED --> */}
<Accordion title="Forma de respuesta: sync vs async">
<Tabs>
  <Tab title="Respuesta sync">
```json
{
  "id": "msg_abf981cf3f1c449eacb7a85c064ca505",
  "type": "message",
...
- **Falta de auth**: incluye `Authorization: Bearer ...`.
- **Base URL incorrecta**: usa `https://{BASE_URL}/api/gpt/v1/`.
- **Body inválido**: usa `POST /messages/validate` para validar.
- **Paradas o errores inesperados**: revisa [Handling stop reasons](/es/developer-manual/build-with-zylon/zylon-gpt-api/handling-stop-reasons) para causas y soluciones.

## Próximos pasos

<CardGroup cols={2}>
  <Card title="Tools" icon="toolbox" href="/es/developer-manual/build-with-zylon/tools/introduction-to-tools">
    Deja que Zylon use herramientas built-in y custom durante un mensaje.
  </Card>

  <Card title="Artifacts" icon="file" href="/es/developer-manual/build-with-zylon/working-with-artifacts">
    Ingiera texto, archivos o URIs en colecciones reutilizables.
  </Card>
</CardGroup>
