...
- For **custom tools**, you must provide at least the following fields:
  ...
  - `inputSchema`: defines the structure of the input parameters expected by the tool, typically in JSON Schema format.
...
| Tool                  | Purpose                          |  |
|-----------------------|----------------------------------|-----------|
| `semantic_search_v1`  | Perform semantic search across documents | [Semantic Search Tool](/tools/built-in/semantic_search-tool) |
| `tabular_analysis_v1` | Analyze structured data (CSV, Excel)     | [Tabular Analysis Tool](/tools/built-in/tabular_analysis-tool) |
| `web_extract_v1`      | Extract and analyze web content          | [Web Extract Tool](/tools/built-in/web_extract-tool) |
| `database_query_v1`   | Execute SQL database operations          | [Database Query Tool](/tools/built-in/database_query-tool) |

## Custom Tools

You can create custom tools tailored to your own business logic, workflows, or specific requirements. [Custom Tools](/tools/custom/custom-tools)
