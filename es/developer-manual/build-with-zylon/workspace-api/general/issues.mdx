...
<Warning>
En 1.58, los filtros de creador usan IDs de cuenta y sus respuestas contienen datos de cuenta. Consulta [Migración de API 1.57 → 1.58](/es/developer-manual/build-with-zylon/workspace-api/api-migration-1-57-to-1-58#ids-de-cuenta-y-campos-renombrados) antes de actualizar tu integración.
</Warning>
...
| --- | --- | --- | --- |
| `include` | string | No | Lista separada por comas de `Account` y `Project` para incluir objetos relacionados. |
...
| `created_by` | string | No | IDs de cuenta separados por comas. |
| `projects` | string | No | IDs de proyecto separados por comas. |
| `page`, `page_size`, `after`, `before` | mixed | No | Controles de paginación. |
</Accordion>

```bash
curl "https://{BASE_URL}/api/v1/app/issue?page=1&page_size=20&include=Account,Project&states=NotReviewed,InReview" \
