...
<Note>`Read.All` permits these read-only endpoints only when the owner role also grants the corresponding Backoffice permission. It does not elevate its owner, including a SuperAdmin token. Workspace-only tokens are not enough for these endpoints; interactive sessions are unaffected by token scopes.</Note>
