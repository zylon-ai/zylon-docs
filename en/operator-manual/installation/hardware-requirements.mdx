...
Zylon only has one strict requirement regarding hardware: **it must have access to a GPU with NVIDIA CUDA capabilities and at least 96GB of total VRAM**. Depending on the hardware, some AI models might be restricted, so to ensure compatibility aim for newest compatible CUDA versions (12.6+).
...
In any case, as a direct answer to the question of which GPU should you buy, our current baseline recommendation is a configuration with **at least 96GB of total VRAM**. Some existing client deployments still run Zylon on RTX 5090s supporting 200+ users with good performance, but those setups are below the current minimum recommendation.
...
This reference configuration should be adapted to meet the current minimum GPU baseline of **96GB of total VRAM**. In practice, that means using a GPU such as the **NVIDIA RTX PRO 6000 (Workstation) (96 GB)** or a multi-GPU setup that reaches the same total VRAM, together with a powerful CPU (16 cores), 128 GB of RAM, enough storage capacity to operate Zylon with margin to grow, a robust cooling solution, and a motherboard sized for the selected GPU layout.
