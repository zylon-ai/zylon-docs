...
<Steps>
  ...
  <Step title="Instalar Modelos de IA">
    ...
    **Antes de instalar Zylon**, debes instalar los modelos de IA. Los modelos están determinados por tu configuración de preset de IA y se descargarán desde los dominios de Hugging Face.

    Asegúrate de que tu firewall permita acceso a:
    - `huggingface.co`
    - `xet.huggingface.co`
...
2. **Actualizar configuración**: Ejecuta `sudo zylon-cli config init` para actualizar la configuración para operación online. Asegúrate de configurar según describe el paso [Configurar Zylon](#configure-zylon).
...
4. **Desactivar modo offline**: Edita `/etc/zylon/zylon-conf.yaml` y establece:
    ```yaml
    airgap:
      offline_operation: false
    ```
5. **Refrescar configuración**: Ejecuta `sudo zylon-cli sync` para aplicar los nuevos ajustes de configuración.
6. **Verificar operación**: Confirma que todos los servicios funcionan correctamente.
