---
title: "Artefactos"
description: "Artefactos de Workspace para documentos, agent flows y conectores."
---
...
| --- | --- | --- |
| **Artefactos de documento** | Documentos, carpetas, links | Guardar u organizar contenido en un proyecto. |
| **Agent flows** | Summary, composition, extracción | Ejecutar flujos sobre artefactos para producir resultados. |
| **Otros artefactos** | MCP servers, SQL databases | Conectar herramientas o fuentes externas. |

### Artefactos de documento

Los documentos contienen contenido, los links referencian artefactos existentes y las carpetas organizan artefactos dentro de un proyecto.
...
Los agent flows ejecutan un workflow de Zylon sobre uno o más artefactos para producir un output estructurado.
...
Los artefactos conector almacenan integraciones para herramientas. `private_props` solo se usa en artefactos **MCP** y **SQL database**.
...
| --- | --- |
| `InvalidExtension` | Extensión de archivo no soportada. |
...
| `ParsingFailure` | El extractor no pudo parsear el archivo. |
| `MaxNodes` | Documento demasiado grande/complejo para parseo. |
| `NoValidFile` | No se encontró un archivo válido. |
| `NoValidNodes` | No se pudo extraer contenido parseable. |
| `InternalError` | Fallo inesperado en la ingesta. |
| `Unknown` | Falla no clasificada. |

Si `ingest_status` es `Error`, corrige el archivo (o conviértelo) y re‑sincroniza.
- **Warnings**: `ingest_warnings` puede incluir valores como `BigSize`, `UnprocessableContent` o `NoContent` para indicar problemas no fatales.
