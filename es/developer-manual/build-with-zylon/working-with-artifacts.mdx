...
Un **artefacto** es información (o un recurso listo para herramientas) que ingestas en Zylon para usarla como contexto en consultas y operaciones. Los artefactos viven dentro de una **colección** y cada uno tiene un ID estable.
...
| Tipo | Input | Notas |
| --- | --- | --- |
| **Text** | String | Contenido directo. |
| **File** | Bytes base64 | Para archivos como `.txt` o `.pdf`. |
...
Usa `POST /artifacts/ingest` cuando quieras que la ingesta termine en la misma request.
...
Usa `POST /artifacts/ingest/async` para payloads más grandes o cuando la ingesta pueda tardar. El payload es el mismo que la ingesta síncrona, envuelto en `ingest_body`, y la respuesta devuelve un `task_id` que puedes consultar.
...
- **413**: input demasiado grande para una ingesta.
- **422**: payload inválido (usa `POST /artifacts/ingest/validate` si quieres pre‑validar).
- **Fallos async**: cuando `task_status` es `FAILURE`, revisa `task_result` (por ejemplo `routing_key_error: "ingest.failed"` o `delete.failed`) y reintenta tras corregir el input.
