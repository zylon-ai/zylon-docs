...
``` 
The type of `tool_context` depends on the tool selected, as described in [Context Artifacts](/en/api-reference/tools/introduction-to-tools#context-artifacts) section of the documentation.
...
2. Provide the corresponding `tool_context` with the appropriate artifact type that matches the tool’s requirements (see [Context Artifacts](/en/api-reference/tools/introduction-to-tools#context-artifacts)).
...
| Tool                  | Purpose                          |  |
|-----------------------|----------------------------------|-----------|
| `semantic_search_v1`  | Perform semantic search across documents | [Semantic Search Tool](/en/api-reference/tools/built-in/semantic-search-tool) |
| `tabular_analysis_v1` | Analyze structured data (CSV, Excel)     | [Tabular Analysis Tool](/en/api-reference/tools/built-in/tabular-analysis-tool) |
| `web_extract_v1`      | Extract and analyze web content          | [Web Extract Tool](/en/api-reference/tools/built-in/web-extract-tool) |
| `database_query_v1`   | Execute SQL database operations          | [Database Query Tool](/en/api-reference/tools/built-in/database-query-tool) |

## Custom Tools

You can create custom tools tailored to your own business logic, workflows, or specific requirements. [Custom Tools](/en/api-reference/tools/custom/custom-tools)
