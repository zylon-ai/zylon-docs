...
This problem this sample tries to showcase is transforming an old file
(like a scanned PDF) into a structured JSON format that can later be post processed
for other automation tools.

In particular, this scripts extracts information from a citizenship application form
and transforms it into a structured JSON object.

You can modify the fields to extract and the JSON schema to fit your needs.

To run this simply you will need:
* Python 3 (no external dependencies)
* An API token from your Zylon instance (see [Token Management](/api-reference/token-management) for more details)
