...
| Requirement | Value |
| --- | --- |
| Authentication | Account token with `Read.All` or `ReadWrite.All`, or an account session |
| Role | The token or session owner must have at least the `operator` role |
...
<Note>`Read.All` permits these read-only endpoints only when the owner role also grants the corresponding Backoffice permission. It does not elevate its owner, including a SuperAdmin token. Workspace-only and gateway-scoped ZylonGPT tokens are not enough for these endpoints; interactive sessions are unaffected by token scopes.</Note>
...
| `ws.member.create` | Workspace | Workspace member created. |
...
| `ws_member_id` | Workspace-member context when available. |
  ...
  "data": [
    {
      ...
      "ws_member_id": "01985a4e-9811-7000-8004-d8f56e017c11",
