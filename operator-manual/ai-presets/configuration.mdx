...
Check current shared memory utilization:

```bash
# View pod memory usage
kubectl top pods -l app=triton-inference
...
**Symptom: `Shared memory allocation failed` errors**

1. Check current shared memory limit in configuration
2. Review inference request patterns and sizes
3. Increase limit incrementally (2Gi → 4Gi → 8Gi)
4. Monitor for OOMKilled events after changes
5. Verify sufficient system RAM is available

**Symptom: Pods getting OOMKilled after increasing shared memory**

1. Reduce shared memory allocation
2. Check total system RAM availability
3. Review other memory-intensive processes
4. Consider upgrading system RAM or optimizing workloads
