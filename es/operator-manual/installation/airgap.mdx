...
<Steps>
  ...
  <Step title="Importar Modelos de IA">
    ...
    **Antes de importar Zylon**, importa los modelos de IA que fueron exportados en la máquina online:
    ...
    ```bash
    cd /tmp
    sudo zylon-cli import zylon-models-<timestamp>.tar
    ```
  </Step>

  <Step title="Instalar Modelos de IA">
    _Tiempo estimado: 2 a 10 min_

    Después de importar los modelos, instálalos:

    ```bash
    sudo zylon-cli models install
    ```

    Esto instalará los modelos de IA que fueron importados en el paso anterior.
