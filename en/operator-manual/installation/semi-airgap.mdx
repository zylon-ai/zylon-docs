...
<Info>
  Since Zylon relies on container images, network access to original registries is required during installation and updates.
  If your security policies restrict access to container registries, consider move to an airgap installation using [Zylon Airgap Installation Guide](/en/operator-manual/installation/airgap).
</Info>
...
<Steps>
  <Step title="Install Zylon CLI">
    ...
    <Warning>
      Before to start the installation, we will validate that we have connectivity to download and install the Zylon CLI.

      ```bash
      curl -sLv get.zylon.ai
      ```
