...
Zylon only requires internet connection for license verification, for that reason

The host `api.replicated.com` must be reachable at all times.

If you plan to run Zylon
fully offline after installation, an offline license must be obtained from the Zylon team,
